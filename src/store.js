...
let slice1 = createSlice({
  ...
  initialState: "초기값",
  reducers: {
    //리덕스에 등록된 슬라이스 값을 변경하고자 하면
    //store에서 변경하고자 하는함수로 등록
    //사용하는 곳에서 그 함수를 사용하는 식으로 변경(간단히 대입x)
    changeData(state, actions) {
      //store : 기존의 initialState 값이 들어있음
...
//slice1을 변경하는 함수를 내보낸다 (useDispatch로 사용가능)
...
//configureStore 에서 내보내줘야 한다.
export default configureStore({
  reducer: {
    // 스테이트를 다른곳에서 사용할 수 있게 등록하는 곳
